...
namespace MyFinance.Worker.Consumers
{
    ...
    public class LancamentoCriadoConsumer : IConsumer<LancamentoCriadoEvent>
    {
        ...
        public async Task Consume(ConsumeContext<LancamentoCriadoEvent> context)
        {
            ...
            // Apenas loga ou faz ações secundárias (ex: enviar email de alerta se for despesa alta)
            Console.WriteLine($"[WORKER] Evento recebido: Lançamento {evento.Id} no valor de {evento.Valor} criado para Conta {evento.ContaId}.");

            await Task.CompletedTask;
        }


        //Comentado pois a atualização precisa ser imediata!!
        ...
        //        // 3. Salva a alteração
        //        await _contaRepository.UpdateAsync(evento.ContaId, conta, cancellationToken);
